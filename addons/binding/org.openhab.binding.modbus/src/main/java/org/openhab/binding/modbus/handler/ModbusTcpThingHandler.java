 ...
 */
public class ModbusTcpThingHandler extends BaseThingHandler implements ModbusEndpointThingHandler {
    ...
    public ModbusTcpThingHandler(Thing thing, ModbusManager manager) {
        super(thing);
    ...
    public void initialize() {
        ...
        // Configuration might have changed, refresh polling
        refreshPollers();
    }

    private void refreshPollers() {
        ((Bridge) getThing()).getThings().stream()
                .filter(thing -> thing.getHandler() != null && thing.getHandler() instanceof ModbusPollerThingHandler)
                .forEach(thing -> ((ModbusPollerThingHandler) thing.getHandler()).initPolling());
    }
