...
public class SmokeTest extends IntegrationTestSupport {
    ...
    // } catch (NoSuchMethodException e) {
    // // In newer Javas (Java 14->) SocksSocketImpl is inheriting from DelegatingSocketImpl
    // // which is wrapping the actual socket
    // try {
    // Class<?> delegatingSocketImplClass = Class.forName("java.net.DelegatingSocketImpl");
    // Method getDelegate = delegatingSocketImplClass.getDeclaredMethod("delegate");
    // getDelegate.setAccessible(true);
    // return (Socket) getDelegate.invoke(impl);
